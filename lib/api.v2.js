...
function applyParams(collection, params) {
  if (params.length < 1) return collection.data;
  var chain = collection.chain();
  _.each(params, chain.find, chain);
  return chain.data();
}

// Partition params to [<nameQuery>, [<restQueries>]]
// in its own function for when version is added
function partitionParams(actionParams) {
  var params = _.partition(actionParams, "name");
  var name = params[0].length ? params[0][0].name : null;
...
module.exports.processRequest = function (collection, query, action, exactMatch, cb) {
  ...
  switch (action) {
    case this.actions.find:
      _find(collection, query, actionParams, exactMatch, cb);
      break;
    case this.actions.findOne:
      _findOne(collection, query, actionParams, exactMatch, cb);
      break;
    default:
...
function _find(collection, query, actionParams, exactMatch, cb) {

  // Filter it down to the meat of the query
  var params = partitionParams(actionParams, "name"),
    nameCriteria = params[0], otherCritieria = params[1],
    libs = applyParams(collection, otherCritieria);
...
function _findOne(collection, query, actionParams, exactMatch, cb) {
  var params = partitionParams(actionParams, "name"),
    nameCriteria = params[0], otherCritieria = params[1],
    libs = applyParams(collection, otherCritieria);
