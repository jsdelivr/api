...
var _ = require('lodash')
  ...
  , dbs = require('../db')
  , buildActionParams = require('./buildActionParams');
var APIError = require('./APIError');

const defaultFields = Object.keys(dbs._v1Schema);
...
module.exports.processRequest = function (collection, query, action, cb) {
  ...
  var actionParams = buildActionParams('v1', query);
...
function _find(collection, query, actionParams, ops, cb) {
  ...
  if (!_.has(ops, op)) {
    var error = new APIError('Invalid operation type supplied: ' + op, 400);
    return cb(error);
  }
