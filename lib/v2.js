...
const minimatchProperties = [ 'author', 'lastversion', 'mainfile' ];
const exactMatchProperties = [ 'github', 'homepage' ];
...
export function findOne (collection, query, cb) {
	...
	if (query.version) {
		let files = selectVersionAssets(query.version, model);
...
function applyParams (collection, query) {
	let params = _.mapValues(_.pick(query, minimatchProperties), (val) => {
		return { '$regex': minimatch.makeRe(val, { nocase: true }) };
	});

	_.extend(params, _.mapValues(_.pick(query, exactMatchProperties), val => {
		return { '$eq': val };
	}));
...
function selectVersionAssets (version, model) {
	return (_.find(model.assets, (value, key) => key === version));
}
