...
export function find (collection, query, cb) {
	...
	if (!_.has(ops, op)) {
		return cb(new ApiError(`Invalid operation type supplied: ${op}`, 400));
	}
	...
	if (actionParams.length > 1 && op === ops.and) {
		...
		return cb(null, selectFields(query.fields, chain.data()));
	}
	...
	// or
		...
		return cb(null, selectFields(query.fields, _.flatten(_.map(actionParams, (actionParam) => {
	...
	// one parameter
		...
		return cb(null, selectFields(query.fields, collection.find(actionParams[0])));
	}

	// no parameters
	else {
		return cb(null, selectFields(query.fields, collection.data));
	}
}
